# Cytoscape figure for cassava fingerprinting data
# For max relationships between farm x reference

setwd("~/Documents/cassava_fingerprinting/")
library(dplyr)
library(readr)

# Read BOTH files and combine them into one complete dataset
max_ref_temp <- read.csv("max_ref_relationships.csv")
ref_ref_temp <- read.csv("ref_ref_relationships.csv")

# Combine both files into one complete max_ref_relationships
...
vis_edges <- edges %>%
  # Filter for stronger relationships to avoid clutter
