#!/usr/bin/env Rscript
# Convert DArTseq-LD csv file (*SNP_mapping_2.csv) to VCF format
...
fix_csv_header <- function(csv_file) {
  ...
  if (length(duplicate_names) > 0) {
    ...
    # Rename all occurrences of duplicate names (add column number for tracking)
...
# Then modify the beginning of the function:
snp_mapping_to_vcf_fixed <- function(snp_file, output.file, ploidy = 2) {
  ...
  # Sort data by chromosome and position for VCF format
  ...
  # Add sample header with fixed sample names
